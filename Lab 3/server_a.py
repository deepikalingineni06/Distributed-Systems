...
def impl_cmd(command=None):
    """Runs the command queried by client"""
    RD_ONLY, RW_ONLY = 0o444, 0o666
    file_mode, file_key = command.split()
    try:
        file_key = int(file_key)
        file = sorted(ls(directory=WATCH_DIR))[file_key].split()[0]
    except:
        print("[WARN]:", "File key/index must be an integer!", file=stderr)
        return None

    # Handle file operation
    if file_mode == "lock":
        os.chmod(os.path.join(WATCH_DIR, file), RD_ONLY)
    elif file_mode == "unlock":
        os.chmod(dst:=os.path.join(WATCH_DIR, file), RW_ONLY)
        src = os.path.join(REMOTE_DIR, file)
        if os.path.exists(src):
            shutil.copyfile(src, dst)
        else:
            os.remove(dst)
        print("[+] {file} updated - QUEUE popped!")
    else:
        print("[WARN]:", "Something went wrong!", file=stderr)
        return None
    ...
    with socket(AF_INET, SOCK_STREAM) as client_sock:
        ...
        try:
            ...
            while True:
                ...
                client_cmd = connection.recv(BUFF).decode()
                if client_cmd != "X":
                    impl_cmd(command=client_cmd) # Implements the [client command]
